...
func DialHTTPWithClient(endpoint string, client *http.Client) (*Client, error) {
	...
	return newClient(context.Background(), fn, &cfg)
}
...
func (c *Client) sendHTTP(ctx context.Context, op *requestOp, msg interface{}, logger Logger, debug bool) error {
	hc := c.writeConn.(*httpConn)
	respBody, err := hc.doRequest(ctx, msg, logger, debug)
	if err != nil {
		return err
	}
...
func (c *Client) sendBatchHTTP(ctx context.Context, op *requestOp, msgs []*jsonrpcMessage, logger Logger, debug bool) error {
	hc := c.writeConn.(*httpConn)
	respBody, err := hc.doRequest(ctx, msgs, logger, debug)
	if err != nil {
		return err
	}
	defer respBody.Close()
...
func (hc *httpConn) doRequest(ctx context.Context, msg interface{}, logger Logger, debug bool) (io.ReadCloser, error) {
	...
	if debug {
		logger.Infof("req %s content length %f KB", req.URL, req.ContentLength/1024)
	}
	// set headers
	hc.mu.Lock()
