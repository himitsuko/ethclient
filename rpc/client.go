...
func DialOptions(ctx context.Context, rawurl string, options ...ClientOption) (*Client, error) {
	...
	return newClient(ctx, reconnect, cfg)
}
...
func newClient(initctx context.Context, connect reconnectFunc, clientCfg *clientConfig) (*Client, error) {
	...
	if clientCfg != nil {
		...
		debug = clientCfg.debug
	}
	c := initClient(conn, randomIDGenerator(), new(serviceRegistry), logger, debug)
	c.reconnectFunc = connect
	return c, nil
}

func initClient(conn ServerCodec, idgen func() ID, services *serviceRegistry, logger Logger, debug bool) *Client {
...
func (c *Client) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	if c.isHTTP {
		err = c.sendHTTP(ctx, op, msg, c.logger, c.debug)
	} else {
...
func (c *Client) BatchCallContext(ctx context.Context, b []BatchElem) error {
	...
	var err error
	if c.isHTTP {
		err = c.sendBatchHTTP(ctx, op, msgs, c.logger, c.debug)
	} else {
...
func (c *Client) Notify(ctx context.Context, method string, args ...interface{}) error {
	...
	if c.isHTTP {
		return c.sendHTTP(ctx, op, msg, c.logger, c.debug)
	}
